...
var (
	// Nas Stats
	rubrikNasCapacityLocalUsed = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
			Name: "rubrik_nas_capacity_local_used_bytes",
			Help: "Local storage consumption for NAS snapshots.",
		},
		[]string{
			"clusterName",
			"objectName",
...
// GetNasCapacityStats ...
func GetNasCapacityStats(rubrik *rubrikcdm.Credentials, clusterName string) {
	...
	for {
		...
		for _, v := range dataGrid {
			...
			rubrikNasCapacityLocalUsed.WithLabelValues(
				clusterName,
				thisObjectName,
				thisObjectID,
				thisLocation).Set(thisLocalStorage)
			rubrikNasCapacityArchiveUsed.WithLabelValues(
				clusterName,
				thisObjectName,
				thisObjectID,
